...
public class Conditional {
	public static void main(String[] args) {
		...
		// 로그인 하지 않은 화면을 보여 줍니다.

		// jsp 때는 세션에서 정보를 가져 온다.
		String loginId = "akow283";

		if(loginId != null) {
			System.out.println("로그인 상태의 화면을 보여 준다.");
		}else {
			System.out.println("로그인 화면으로 이동 시켜 준다.");
		}


		// 리모콘의 전원 버튼의 경우 버튼 하나로
		// on/off가 다 가능합니다.
		// 해당 버튼 이벤트에 전원의 상태를 체크하는 기능이 있겠죠?

		boolean isPowerOn = false;

		// 전원 버튼을 눌렀을 때의 로직

		if(isPowerOn) {	// ispowerOn이 true면 TV가 켜져 있다.
			System.out.println("TV를 끕니다.");
			isPowerOn = false;
		}else {	// ispoweOn false면 TV가 꺼져 있다.
		...
		// switch문
		// switch() 괄호 안에는 char, byte, int, string 타입의 변수만
		// 적용이 가능하다.
