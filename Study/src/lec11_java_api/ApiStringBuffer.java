...
public class ApiStringBuffer {
	public static void main(String[] args) {
		...
		// 그로 인해 가비지컬렉터가 할 일이 많아집니다.

		// 1부터 10까지의 숫자를 문자열로 다 더하면?
		String strVal = "";
		for (int i = 1; i <= 10; i++) {
			strVal += i;
			// 매 순간 새로운 메모리 영역을 할당 받으며,
			// 기존 메모리 영역에 남아있는 값은 가비지컬렉터에 의해
			// 삭제된다. (비효율)
		}
		...
		// StringBuilder 가 가진 문자열을 String으로 변환
		...
		// 동기화 미지원
		// 쓰레드 a, b, c 에서 strBuild를 사용할 때, 쓰레드 a에서
		// strBuild를 바꾼 경우 b, c는 이를 모른다.
		// 단일 쓰레드를 이용한다면 좋은 성능
		
		// 동기화 지원
		// 쓰레드 a, b, c 에서 strBuild를 사용할 때, 쓰레드 a에서
		// strBuild를 바꾼 경우 b, c는 이를 알 수 있다.
		// 단일 쓰레드를 이용한다면 느린 성능
	}
}
