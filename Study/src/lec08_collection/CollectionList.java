...
public class CollectionList {
	public static void main(String[] args) {
		...
		// Wrapper class는 기본 타입의 데이터를 객체로 취급해야
		// 하는 경우 사용한다.
		// 기본 타입 = 숫자, boolean
		int intVal = 12;
		
		// 박싱(boxing)
		// 기본 타입의 데이터(12)를 래퍼 클래스의 인스턴스려 변환
		Integer num = new Integer(12);
		// 오토 박싱(auto boxing)
		Integer num2 = 12;
		
		// 언박싱(Unboxing)
		// 래퍼 클래스의 인스터스에 저장된 값을 기본 타입의 데이터로 꺼냄
		int n = num.intValue();
		// 오토 언박싱
		int n2 = num;
...
//		System.out.println(strArray.length);
		...
		System.out.println(students);
		students.remove("파이리"); // 해당 데이터가 없어도 됨
		...
		intVal = 10;	// 변수 초기화 
		students = new ArrayList<String>();	// 초기화
		System.out.println(students);
	
		students.add("범석"); // 한 값만 넣을 수 있음
		students.add("가은");
		students.add("상우");
		students.add("용석");
...
//					intList[j+1] = temp;
		...
		Collections.sort(strList);	// 원본이 바뀜
