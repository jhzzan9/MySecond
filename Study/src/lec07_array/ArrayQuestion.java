...
public class ArrayQuestion {
	public static void main(String[] args) {
		...
		System.out.println("최대값은 : " + maxVal);
		...
		//	  최댓값을 찾아 해당 위치의 값과 배열의 끝에서 두번째 값과 swap 한다.
		
		// @. 최댓값을 이용해서 내림차순으로 해보세요.
		...
		for (int i = 0; i < intArray.length - 1; i++) {
			...
			for (int j = 1; j < intArray.length - i; j++) {
				if (maxNum < intArray[j]) {
					maxNum = intArray[j];
					maxIdx = j;
				}
			}
			// swap, 최대값인 maxNum의 인덱스와
			// 바꿀 자리인 인덱스 자리
