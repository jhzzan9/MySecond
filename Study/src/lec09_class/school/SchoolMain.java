...
public class SchoolMain {
	...
	public static void main(String[] args) {
		...
		// 값을 초기화하지 않았다면, 필드 변수들은
		// 초기 값들로 설정되어 있음
		// 기본 타입(숫자, boolean), 참조타입
		// 숫자는 0 (실수는 0.0)
		// boolean은 false
		// 참조타입은 null
		...
		bumseok.eng = 83;
		bumseok.math = 88;
		bumseok.avg = (bumseok.kor + bumseok.eng + bumseok.math) / 3;
		bumseok.isMan = true;
		...
		// 필드값을 입력받는 생산자(construstor)로 객체 만들기
		...
		// syso 로 객체 표현하기
		// 클래스 내에서 toString을 @Override 하면 됨
		...
		for (int i = 0; i < stuList.size(); i++) {
			...
			for (int j = 0; j < stuList.size() - 1; j++) {
				// 정렬 순서 바꾸기
				// 아래 if문 부등호만 바꾸면 끝!!
				if (stuList.get(j).avg < stuList.get(j + 1).avg) { // 민우, 범석 중 평균 값이 큰 사람 구하기
					Student temp = stuList.get(j);
					stuList.set(j, stuList.get(j + 1)); // set(인덱스, 인덱스 값)
